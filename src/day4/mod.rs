...
fn is_range_overlap(range1: &RangeInclusive<u64>, range2: &RangeInclusive<u64>) -> bool {
   range1.contains(range2.start()) || range1.contains(range2.end()) || range2.contains(range1.start()) || range2.contains(range1.end())
}
...
pub fn part2(input: String) -> usize {
    input.lines()
        .filter(|&line| line.split(',')
            ...
            .permutations(2)
            .any(|permutation| {
                is_range_overlap(&permutation[0], &permutation[1])
            })).count()
}
...
mod tests {
    ...
    fn test_part1() {
        ...
        let input = std::fs::read_to_string("input/day4/example.txt").expect("Can't load example file");
