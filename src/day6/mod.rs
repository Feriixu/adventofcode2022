...
mod tests {
    ...
    fn test_part1() {
        ...
        let input =
            std::fs::read_to_string("input/day6/example.txt").expect("Can't load example file");
        for line in input.lines() {
            let (stream, solution) = line.split_whitespace().collect_tuple().unwrap();
            assert_eq!(
                part1(stream.to_string()),
                solution.parse::<usize>().unwrap()
            );
        }
    }
    ...
    fn test_part2() {
        // Load the example input
        let input =
            std::fs::read_to_string("input/day6/example2.txt").expect("Can't load example file");
        for line in input.lines() {
            let (stream, solution) = line.split_whitespace().collect_tuple().unwrap();
            assert_eq!(
                part2(stream.to_string()),
                solution.parse::<usize>().unwrap()
            );
        }
    }
